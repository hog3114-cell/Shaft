...
# ------------------------------------------------------------
# [2] 마감 시간 & 구글 시트 연결 (한국시간 KST 기준)
# ------------------------------------------------------------
KST = datetime.timezone(datetime.timedelta(hours=9))
DEADLINE = datetime.datetime(2026, 6, 25, 10, 45, 0, tzinfo=KST)
...
def save_pick(name, picks):
    """이름 기준 upsert. 동시 제출 충돌을 줄이기 위해 저장 직전에
    시트를 다시 읽어 최신 상태에 합치고, 실패 시 여러 번 재시도한다."""
    import time, random
    last_err = None
    for attempt in range(5):  # 최대 5회 재시도
        try:
            df = conn.read(worksheet="picks", ttl=0)  # 저장 직전 최신본 다시 읽기
            df = df.dropna(how="all")
        except Exception:
            df = pd.DataFrame(columns=["name"] + list(QUESTION_MAP.keys()) + ["updated_at"])

        row = {"name": name, **picks, "updated_at": now.strftime("%Y-%m-%d %H:%M:%S")}
        if not df.empty and name in df["name"].values:
            df = df[df["name"] != name]  # 같은 이름이면 기존 행 교체(수정)
        ...
        try:
            ...
            last_err = e
            time.sleep(0.4 + random.random() * 0.6)  # 살짝 대기 후 재시도
    raise last_err if last_err else RuntimeError("저장 실패")
...
st.markdown('<div class="bm-subbar"><span>⚽ 이벤트 승부식</span><span class="bm-deadline">마감 6/25(목) 10:45</span></div>', unsafe_allow_html=True)
        ...
        if user_name:
            ...
            if st.button("✅ 최종 조합 구매 (픽 제출)", type="primary", use_container_width=True):
                try:
                    ...
                    st.success(f"{user_name}님 픽이 저장되었습니다! 마이페이지에서 확인하세요.")
                except Exception as e:
...
with tab_admin:
    ...
    if st.button("🏆 자동 채점 및 순위 발표", type="primary", use_container_width=True):
        if df.empty:
            ...
            st.balloons()
            st.success("채점 완료! '🏆 순위발표' 탭에서 결과를 확인하세요.")
